...
int32_t  move(struct Position* position, struct Velocity* velocity){
	...
	printf("sinTheta: 0x%"PRIX64"\tcosTheta: 0x%"PRIX64"\n", sinTheta, cosTheta);
	printf("sinPhi  : 0x%"PRIX64"\tcosPhi  : 0x%"PRIX64"\n", sinPhi  , cosPhi  );

	printf("position.x before: 0x%"PRIX64"\n", position->x);
	position->x += cordic_hi_mul( cordic_hi_mul(velocity->r,sinTheta) << 2 , cosPhi) << 2;
	printf("position.x after:  0x%"PRIX64"\n", position->x);

	printf("position.y before: 0x%"PRIX64"\n", position->y);
	position->y += cordic_hi_mul( cordic_hi_mul(velocity->r,sinTheta) << 2 , sinPhi) << 2;
	printf("position.y after:  0x%"PRIX64"\n", position->y);
	
	printf("position.z before: 0x%"PRIX64"\n", position->z);
	position->z += cordic_hi_mul( velocity->r, cosTheta) << 2;
	printf("position.z after:  0x%"PRIX64"\n", position->z);
	return 0;
}
